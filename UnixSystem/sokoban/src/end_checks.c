...
static char *get_wall_str(char **map, int row, int col)
{
    ...
    char up = map[(row > 0) ? row - 1 : 0][col];
    char right = map[row][(col + 1 >= my_strlen(map[row])) ? col : col + 1];
    char down = map[(row >= my_arraylen(map) - 1) ? row : row + 1][col];
    char left = map[row][(col <= 0) ? 0 : col - 1];
